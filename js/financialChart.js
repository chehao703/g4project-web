function plotFinancial(stockid) {
  var data_cash_div = [];
  var data_stock_div = [];
  ...
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_cash_div);
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_stock_div);
    ...
    data: [
      {
        ...
        yValueFormatString: "###,###,### 百萬元",
        type: "line",
        showInLegend: true,
        name: "利息淨收益",
        markerType: "square",
        color: "#F08080",
        dataPoints: data_interest_income,
      },
      {
        xValueFormatString: "####年",
        yValueFormatString: "###,###,### 百萬元",
      ...
      },
      {
        ...
        name: "淨收益",
        lineDashType: "dash",
        dataPoints: all_income,
      },
    ],
  });

  function adddata_interest_income(data) {
    for (var i = 0; i < data.length; i++) {
      if (data[i].公司 == stockid) {
        data_interest_income.push({
          x: data[i].年,
          y: data[i].利息淨收益 / 1000,
        });
      }
  ...
  function adddata_other_income(data) {
    for (var i = 0; i < data.length; i++) {
      if (data[i].公司 == stockid) {
        other_income.push({
          x: data[i].年,
          y: data[i].利息以外淨損益 / 1000,
        });
      }
    }
    chart_profit.render();
  ...
  function adddata_all_income(data) {
    for (var i = 0; i < data.length; i++) {
      if (data[i].公司 == stockid) {
        all_income.push({
          x: data[i].年,
  ...
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_interest_income);
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_other_income);
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_all_income);
  ...
  $.getJSON("./dataset/個股資訊/個股_金控.json", addData_income);
  $.getJSON("./dataset/個股資訊/個股_金控.json", addData_eps);
  ...
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_assets);
  $.getJSON("./dataset/個股資訊/個股_金控.json", adddata_liabilities);
  ...
  $.getJSON("./dataset/個股資訊/個股_金控.json", addData_net_asset);
  $.getJSON("./dataset/個股資訊/個股_金控.json", addData_bvps);
